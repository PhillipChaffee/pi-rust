...
//! cancellation wiring over `tokio_util`'s `CancellationToken` (upstream's
//! `AbortSignal` machinery, including the fan-in and masking behaviors), and
//! running a future inside a context with `awaitWithContext`.
